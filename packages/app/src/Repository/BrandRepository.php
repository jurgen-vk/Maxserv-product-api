...
class BrandRepository
{
  ...
  public function findOrCreateOneByName(string $name): Brand
  {
    $stmt = $this->connection->pdo->prepare(
      query: 'INSERT INTO brands (name) VALUES (:name) ON DUPLICATE KEY UPDATE id = LAST_INSERT_ID(id)'
    );
  ...
  public function findMany(array $ids): array
  {
    ...
    $stmt = $this->connection->pdo->prepare(query: "SELECT * FROM brands WHERE id IN ($placeholders)");
  ...
  {
    $stmt = $this->connection->pdo->query(query: 'SELECT * FROM brands ORDER BY name');

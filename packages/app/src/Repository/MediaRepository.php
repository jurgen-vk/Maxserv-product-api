...
class MediaRepository
{
  public function __construct(
    ...
    private readonly MediaHydrator $hydrator,
  ) {
  }
  ...
  public function saveMany(array $media): void
  {
    ...
    $values = [];
    foreach ($media as $m) {
      array_push($values, $m->id, $m->entityType, $m->entityId, $m->url, $m->mediaType->value, $m->sortOrder);
    }
  ...
  public function deleteByEntities(string $entityType, array $entityIds): void
  {
    ...
    $this->connection->pdo->prepare(
      query: "DELETE FROM media WHERE entity_type = ? AND entity_id IN ($placeholders)"
  ...
  public function findByEntity(string $entityType, int $entityId): array
  {
    $stmt = $this->connection->pdo->prepare(
      query: 'SELECT * FROM media WHERE entity_type = :entityType AND entity_id = :entityId ORDER BY sort_order'
    );
